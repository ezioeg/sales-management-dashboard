...
const Vendedor = ({
  vendedor,
  mostrarFormVendedor,
  mostrarModalEliminarVendedor,
}) => {
  const { seleccionarVendedor } = useContext(GeneralContext);
      ...
      <tbody>
        <tr className="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
          ...
          <td className="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static text-sm  ">
            ...
            <button
              className=" px-3 py-2 font-bold mr-20 border-red-600 border text-red-600 rounded transition duration-300 hover:bg-red-600 hover:text-white focus:outline-none"
              onClick={() => {
                seleccionarVendedor(vendedor);
                mostrarModalEliminarVendedor();
              }}
