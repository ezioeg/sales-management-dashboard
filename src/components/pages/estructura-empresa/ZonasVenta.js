...
import Zona from "../../ui/estructura-empresa/Zona";
...
const ZonasVenta = () => {
  ...
  const { zonaseleccionada, seleccionarZona } = useContext(GeneralContext);
  ...
  useEffect(() => {
    function manejarSnapshot(snapshot) {
      ...
      // Almacenar los zonas en el estado
      setZonasInfo(zonas);
    }
  ...
  const formik = useFormik({
    ...
    initialValues: {
      id:
        !!zonaseleccionada && !!zonaseleccionada.id ? zonaseleccionada.id : "",
      descripcion:
        !!zonaseleccionada && !!zonaseleccionada.descripcion
    ...
    onSubmit: (zona, { resetForm }) => {
      try {
        zonaseleccionada !== null ? editarZona(zona) : agregarZona(zona);
  ...
  const agregarZona = async (zona) => {
    ...
    await firebase.db
      .collection("zonas")
      .doc(zona.id)
      .set({ descripcion: zona.descripcion });
  ...
  const editarZona = async (zona) => {
    ...
    await firebase.db
      .collection("zonas")
      .doc(zona.id)
      .update({ descripcion: zona.descripcion });
  ...
  const eliminarZona = async () => {
    // try {
    await firebase.db.collection("zonas").doc(zonaseleccionada.id).delete();

    hideModalEliminarZona();
    // } catch (error) {
    //   console.log(error);
    // }
  };

  const mostrarFormZona = () => {
    // Esconde el formulario
  ...
  function showModalEliminarZona() {
    setVisibleModalEliminar(true);
  }

  function hideModalEliminarZona() {
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="flex justify-center ">
              <input
                ...
                value={zonaseleccionada ? "Editar zona" : "Agregar zona"}
              />
            </div>
          </form>
        </div>
        ...
        {search(zonasInfo)
          ...
          .map((zonasInfo) => (
            <Zona
              key={zonasInfo.id}
              zona={zonasInfo}
              mostrarFormZona={() => mostrarForm()}
              mostrarModalEliminarZona={() => showModalEliminarZona()}
            />
          ))}
      </table>
