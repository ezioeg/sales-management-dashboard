...
import Via from "../../ui/estructura-datos-maestros/Via";

const ViasPago = () => {
  const [viasInfo, setViasInfo] = useState([]);
  const [visibleForm, setVisibleForm] = useState(false);
  const [visibleModalEliminar, setVisibleModalEliminar] = useState(false);
  const [q, setQ] = useState("");
  const { viaseleccionada, seleccionarVia } = useContext(GeneralContext);

  useEffect(() => {
    function manejarSnapshot(snapshot) {
      const vias = snapshot.docs.map((doc) => {
        return {
          id: doc.id,
          ...doc.data(),
        };
      ...
      // Almacenar los vias en el estado
      setViasInfo(vias);
    }
  ...
  }, [viaseleccionada]); //viaseleccionada

  // Validacion y leer los datos del formulario
  const formik = useFormik({
    enableReinitialize: true,
    initialValues: {
      id: !!viaseleccionada && !!viaseleccionada.id ? viaseleccionada.id : "",
      descripcion:
        !!viaseleccionada && !!viaseleccionada.descripcion
          ? viaseleccionada.descripcion
          : "",
    },
    ...
    onSubmit: (via, { resetForm }) => {
      try {
        viaseleccionada !== null ? editarVia(via) : agregarVia(via);
  ...
  const agregarVia = async (via) => {
    ...
    await firebase.db
      .collection("vias")
      .doc(via.id)
      .set({ descripcion: via.descripcion });

    // Redireccionar
    // navigate("/"); // via
  };

  const editarVia = async (via) => {
    // console.log(viaId);

    await firebase.db
      .collection("vias")
      .doc(via.id)
      .update({ descripcion: via.descripcion });
  ...
  const eliminarVia = async () => {
    // try {
    await firebase.db.collection("vias").doc(viaseleccionada.id).delete();

    hideModalEliminarVia();
    // } catch (error) {
    //   console.log(error);
    // }
  };

  const mostrarFormVia = () => {
    // Esconde el formulario
  ...
  function showModalEliminarVia() {
    setVisibleModalEliminar(true);
  }

  function hideModalEliminarVia() {
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="flex justify-center ">
              <input
                ...
                value={viaseleccionada ? "Editar vía" : "Agregar vía"}
              />
            </div>
          </form>
        </div>
        ...
        {search(viasInfo)
          ...
          .map((viasInfo) => (
            <Via
              key={viasInfo.id}
              via={viasInfo}
              mostrarFormVia={() => mostrarForm()}
              mostrarModalEliminarVia={() => showModalEliminarVia()}
            />
          ))}
      </table>
