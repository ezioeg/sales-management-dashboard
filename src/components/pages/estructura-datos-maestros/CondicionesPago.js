...
import Condicion from "../../ui/estructura-datos-maestros/Condicion";
...
const CondicionesPago = () => {
  ...
  const { condicionseleccionada, seleccionarCondicion } = useContext(
    GeneralContext
  );
  ...
  useEffect(() => {
    function manejarSnapshot(snapshot) {
      ...
      // Almacenar los condiciones en el estado
  ...
  const formik = useFormik({
    ...
    initialValues: {
      id:
        !!condicionseleccionada && !!condicionseleccionada.id
          ...
          : "",
      descripcion:
        !!condicionseleccionada && !!condicionseleccionada.descripcion
          ? condicionseleccionada.descripcion
          : "",
      credito:
        !!condicionseleccionada && !!condicionseleccionada.credito
          ? condicionseleccionada.credito
          : 0,
    },
    validationSchema: Yup.object({
      ...
      credito: Yup.number()
        ...
        .required("El credito es obligatorio"),
    }),
    onSubmit: (condicion, { resetForm }) => {
      try {
  ...
  const agregarCondicion = async (condicion) => {
    // Esconde el formulario
    setVisibleForm(false);

    await firebase.db.collection("condiciones").doc(condicion.id).set({
  ...
  const editarCondicion = async (condicion) => {
    // console.log(condicionId);

    await firebase.db.collection("condiciones").doc(condicion.id).update({
      descripcion: condicion.descripcion,
      credito: condicion.credito,
    });
  ...
  const eliminarCondicion = async () => {
    // try {
    await firebase.db
      .collection("condiciones")
      .doc(condicionseleccionada.id)
      .delete();

    hideModalEliminarCondicion();
    // } catch (error) {
    //   console.log(error);
    // }
  };

  const mostrarFormCondicion = () => {
    // Esconde el formulario
  ...
  function showModalEliminarCondicion() {
    setVisibleModalEliminar(true);
  }

  function hideModalEliminarCondicion() {
    ...
    return rows.filter(
      (row) =>
        ...
        row.descripcion.toString().toLowerCase().indexOf(q.toLowerCase()) >
          -1 ||
        row.credito.toString().toLowerCase().indexOf(q.toLowerCase()) > -1
    );
  }
    ...
    <div className=" ">
          ...
          <form onSubmit={formik.handleSubmit}>
            <div className="-mx-3 md:flex mb-6 ">
              ...
              <div className="md:w-2/5 px-3">
                ...
                {formik.touched.credito && formik.errors.credito ? (
                  <div>
                    <p className="text-red-600 text-xs italic">
        ...
        {search(condicionesInfo)
          ...
          .map((condicionesInfo) => (
            <Condicion
              ...
              condicion={condicionesInfo}
              mostrarFormCondicion={() => mostrarForm()}
              mostrarModalEliminarCondicion={() => showModalEliminarCondicion()}
            />
          ))}
      </table>
      ...
      <div>
        <Rodal
          ...
          onClose={() => hideModalEliminarCondicion()}
          width={300}
          height={150}
        >
