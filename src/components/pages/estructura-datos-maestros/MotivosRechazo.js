...
import Rechazo from "../../ui/estructura-datos-maestros/Rechazo";

const MotivosRechazo = () => {
  const [rechazosInfo, setRechazosInfo] = useState([]);
  const [visibleForm, setVisibleForm] = useState(false);
  const [visibleModalEliminar, setVisibleModalEliminar] = useState(false);
  const [q, setQ] = useState("");
  const { rechazoseleccionado, seleccionarRechazo } = useContext(
    GeneralContext
  );

  useEffect(() => {
    function manejarSnapshot(snapshot) {
      const rechazos = snapshot.docs.map((doc) => {
        return {
          id: doc.id,
          ...doc.data(),
        };
      ...
      // Almacenar los rechazos en el estado
      setRechazosInfo(rechazos);
    }
  ...
  const formik = useFormik({
    ...
    initialValues: {
      id:
        !!rechazoseleccionado && !!rechazoseleccionado.id
          ? rechazoseleccionado.id
          : "",
      descripcion:
        !!rechazoseleccionado && !!rechazoseleccionado.descripcion
          ? rechazoseleccionado.descripcion
          : "",
    },
    ...
    onSubmit: (rechazo, { resetForm }) => {
      try {
        rechazoseleccionado !== null
          ? editarRechazo(rechazo)
          : agregarRechazo(rechazo);
  ...
  const agregarRechazo = async (rechazo) => {
    ...
    await firebase.db
      .collection("rechazos")
      .doc(rechazo.id)
      .set({ descripcion: rechazo.descripcion });
  ...
  const editarRechazo = async (rechazo) => {
    // console.log(rechazoId);

    await firebase.db
      .collection("rechazos")
      .doc(rechazo.id)
      .update({ descripcion: rechazo.descripcion });
  ...
  const eliminarRechazo = async () => {
    // try {
    await firebase.db
      .collection("rechazos")
      .doc(rechazoseleccionado.id)
      .delete();

    hideModalEliminarRechazo();
    // } catch (error) {
    //   console.log(error);
    // }
  };
  ...
  function showModalEliminarRechazo() {
    setVisibleModalEliminar(true);
  }

  function hideModalEliminarRechazo() {
    ...
    <div className=" ">
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="flex justify-center ">
              <input
                ...
                value={
                  rechazoseleccionado ? "Editar rechazo" : "Agregar rechazo"
                }
              />
            </div>
          </form>
      ...
      <table className="border-collapse w-full shadow-xl rounded-lg">
        ...
        {search(rechazosInfo)
          ...
          .map((rechazosInfo) => (
            <Rechazo
              key={rechazosInfo.id}
              rechazo={rechazosInfo}
              mostrarFormRechazo={() => mostrarForm()}
              mostrarModalEliminarRechazo={() => showModalEliminarRechazo()}
            />
          ))}
      </table>
      ...
      <div>
        <Rodal
          ...
          onClose={() => hideModalEliminarRechazo()}
          width={300}
          height={150}
        >
