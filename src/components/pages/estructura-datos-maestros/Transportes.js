...
import Transporte from "../../ui/estructura-datos-maestros/Transporte";

const Transportes = () => {
  const [transportesInfo, setTransportesInfo] = useState([]);
  const [visibleForm, setVisibleForm] = useState(false);
  const [visibleModalEliminar, setVisibleModalEliminar] = useState(false);
  const [q, setQ] = useState("");
  const { transporteseleccionado, seleccionarTransporte } = useContext(
    GeneralContext
  );
  ...
  useEffect(() => {
    function manejarSnapshot(snapshot) {
      ...
      // Almacenar los transportes en el estado
      setTransportesInfo(transportes);
    }
  ...
  }, [transporteseleccionado]); //transporteseleccionado
  ...
  const formik = useFormik({
    ...
    initialValues: {
      id:
        !!transporteseleccionado && !!transporteseleccionado.id
          ? transporteseleccionado.id
          : "",
      descripcion:
        !!transporteseleccionado && !!transporteseleccionado.descripcion
          ? transporteseleccionado.descripcion
          : "",
    },
  ...
  const agregarTransporte = async (transporte) => {
    ...
    await firebase.db
      .collection("transportes")
      .doc(transporte.id)
      .set({ descripcion: transporte.descripcion });

    // Redireccionar
    // navigate("/"); // transporte
  };

  const editarTransporte = async (transporte) => {
    // console.log(transporteId);

    await firebase.db
      .collection("transportes")
      .doc(transporte.id)
      .update({ descripcion: transporte.descripcion });
  ...
  const eliminarTransporte = async () => {
    ...
    await firebase.db
      .collection("transportes")
      .doc(transporteseleccionado.id)
      .delete();

    hideModalEliminarTransporte();
    // } catch (error) {
    //   console.log(error);
    // }
  };

  const mostrarFormTransporte = () => {
    // Esconde el formulario
  ...
  function showModalEliminarTransporte() {
    setVisibleModalEliminar(true);
  }

  function hideModalEliminarTransporte() {
    ...
    <div className=" ">
      ...
      <table className="border-collapse w-full shadow-xl rounded-lg">
        ...
        {search(transportesInfo)
          ...
          .map((transportesInfo) => (
            <Transporte
              key={transportesInfo.id}
              transporte={transportesInfo}
              mostrarFormTransporte={() => mostrarForm()}
              mostrarModalEliminarTransporte={() =>
                showModalEliminarTransporte()
              }
            />
          ))}
      </table>
      ...
      <div>
        <Rodal
          ...
          onClose={() => hideModalEliminarTransporte()}
          width={300}
          height={150}
        >
