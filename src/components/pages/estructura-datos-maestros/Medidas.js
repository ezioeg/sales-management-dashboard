...
import Medida from "../../ui/estructura-datos-maestros/Medida";
...
const Medidas = () => {
  ...
  const { medidaseleccionada, seleccionarMedida } = useContext(GeneralContext);
  ...
  useEffect(() => {
    function manejarSnapshot(snapshot) {
      ...
      // Almacenar los medidas en el estado
      setMedidasInfo(medidas);
    }
  ...
  const formik = useFormik({
    ...
    initialValues: {
      id:
        !!medidaseleccionada && !!medidaseleccionada.id
          ? medidaseleccionada.id
          : "",
      descripcion:
        !!medidaseleccionada && !!medidaseleccionada.descripcion
  ...
  const agregarMedida = async (medida) => {
  ...
  const editarMedida = async (medida) => {
    // console.log(medidaId);

    await firebase.db
      .collection("medidas")
      .doc(medida.id)
      .update({ descripcion: medida.descripcion });
  ...
  const eliminarMedida = async () => {
    // try {
    await firebase.db.collection("medidas").doc(medidaseleccionada.id).delete();

    hideModalEliminarMedida();
    // } catch (error) {
    //   console.log(error);
    // }
  };

  const mostrarFormMedida = () => {
    // Esconde el formulario
  ...
  function showModalEliminarMedida() {
    setVisibleModalEliminar(true);
  }

  function hideModalEliminarMedida() {
    ...
    <div className=" ">
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="flex justify-center ">
              <input
                ...
                value={medidaseleccionada ? "Editar medida" : "Agregar medida"}
              />
            </div>
          </form>
        </div>
        ...
        {search(medidasInfo)
          ...
          .map((medidasInfo) => (
            <Medida
              key={medidasInfo.id}
              medida={medidasInfo}
              mostrarFormMedida={() => mostrarForm()}
              mostrarModalEliminarMedida={() => showModalEliminarMedida()}
            />
          ))}
      </table>
      ...
      <div>
        <Rodal
          ...
          onClose={() => hideModalEliminarMedida()}
          width={300}
          height={150}
        >
